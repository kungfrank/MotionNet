...
#    # Shift padded_voxel_points in z axis
#    padded_voxel_points_tmp = np.copy(padded_voxel_points)
#    padded_voxel_points_new = np.zeros_like(padded_voxel_points) # np.copy(padded_voxel_points)
#    for i in range(12):
#        padded_voxel_points_new[:,:,:,i+1] = padded_voxel_points_tmp[:,:,:,i]
#    padded_voxel_points = torch.tensor(padded_voxel_points_new)

    # Frank Debug Start ##
#    plt.figure('3')
#    plt.imshow(padded_voxel_points[4,:,:,3])
#    plt.figure('4')
#    plt.imshow(padded_voxel_points[4,:,:,4])
#    plt.figure('5')
#    plt.imshow(padded_voxel_points[4,:,:,5])
#    plt.figure('6')
#    plt.imshow(padded_voxel_points[4,:,:,6])
#    plt.figure('7')
#    plt.imshow(padded_voxel_points[4,:,:,7])
#    plt.show()
    # Frank Debug End ##

    padded_voxel_points = torch.unsqueeze(padded_voxel_points, 0).to(device) ################ The only input needed for motionnet
...
#        print(use_adj_frame_pred, use_motion_state_pred_masking)
#        plt.figure(0)
#        plt.imshow(motion_pred_numpy[0])
#        plt.figure(1)
#        plt.imshow(np.sqrt(np.power(disp_pred[19,:,:,0],2)+np.power(disp_pred[19,:,:,1],2)))
#        plt.show()
    ...
    for k in range(len(color_map)):
        ...
        #plt.figure('debug')
        #plt.imshow(np.sqrt(np.power(field_gt[:,:,0],2)+np.power(field_gt[:,:,1],2)))
        ...
        #print(field_pred[:, :, 0].shape)
        #print(field_pred[:, :, 0][mask_pred].shape)
