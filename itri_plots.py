...
img_save_dir = '/mnt/Disk1/viz_result/itri_2021-05-12_14-51-57_ep20_viz4'
num_past_frames = 2
out_seq_len = 1
height_feat_size=1
...
def plot_quiver(ax_, disp0, disp1, viz_cat, viz_motion):
  ...
  thd_mask = field_gt_norm <= 1
  thd_mask = thd_mask * viz_motion
  #field_gt[thd_mask, :] = 0
  # Get the displacement field


  car_mask = np.ones((256,256)).astype(np.bool)
  car_mask[118:138,118:138]=0

#  row_sparse_mask = np.zeros((256,256)).astype(np.bool)
#  col_sparse_mask = np.zeros((256,256)).astype(np.bool)
#  row_sparse_mask[np.arange(0,255,2)] = 1
#  col_sparse_mask[:,np.arange(0,255,2)] = 1
#  sparse_mask = row_sparse_mask * col_sparse_mask
  mask = viz_cat.astype(np.bool) * car_mask * np.logical_not(viz_motion.astype(np.bool)) + thd_mask.astype(np.bool)
  X = idx_x[mask]
  Y = idx_y[mask]
  U = field_gt[:, :, 0][mask]
  V = field_gt[:, :, 1][mask]
  ...
  qk2 = ax_.quiver(Y, X, U, V, angles='xy', scale_units='xy', scale=1, width=0.0005, headwidth=30, headlength=10, headaxislength=10, color=[(0.1,0.15,1.0)], alpha=0.9, minlength=6, minshaft=1)
  # dodgerblue

  mask = viz_cat.astype(np.bool) * car_mask * viz_motion.astype(np.bool) * np.logical_not(thd_mask.astype(np.bool))# * sparse_mask
  X = idx_x[mask]
  Y = idx_y[mask]
  U = field_gt[:, :, 0][mask]
  V = field_gt[:, :, 1][mask]
  qk1 = ax_.quiver(Y, X, U, V, angles='xy', scale_units='xy', scale=0.3, width=0.0011, headwidth=10, headlength=20, headaxislength=20, color='r', alpha=0.9, minlength=4, minshaft=1) #'g'
...
def vis_result(data_path, trained_model_path, which_model, disp, save, ax=-1):
  ...
  for data in data_dirs:
    ...
    if radar_idx < 1620210330.0928211:
      continue
    ...
    raw_radar_power_mask[raw_radar_power_mask>0.12]=1
    raw_radar_power_mask[raw_radar_power_mask<=0.12]=0
    ...
    disp_glo_gt = gt_dict['gt_disp_global']*np.expand_dims(motion_gt, axis=2) * 4.
...
#      viz_motion_pred = outlier_mask
      ...
      #ax[0].imshow(viz_combined(raw_radar, zero_mat, radar_gt*motion_gt))#, cmap='gray')
      ...
      #ax[1].imshow(viz_combined(raw_radar, zero_mat, viz_cat_pred*viz_motion_pred))#, cmap='gray')
      ...
      gt_qk1, gt_qk2 = plot_quiver(ax[0], -disp_glo_gt[:,:,0], -disp_glo_gt[:,:,1], radar_gt, motion_gt)
