...
def calc_err(result, gt, mask, device, class_err_avg, dice_err_avg):
  result = result * mask
  gt = gt * mask

  result = torch.from_numpy(result).to(device).type(torch.FloatTensor)
  gt = torch.from_numpy(gt).to(device).type(torch.FloatTensor)
  # compute class error (BCE) #
  class_err = torch.sum((result-gt)**2) / (mask.sum())
  class_err_avg.update(class_err.item())
  # Dice loss #
  dice_loss = DiceLoss()
  dice_err = dice_loss(result, gt)
  dice_err_avg.update(dice_err.item())

  print(class_err_avg, dice_err_avg)
...
def network_pred(radar_img, model):
  ### input to network
  raw_radars = np.expand_dims(radar_img, axis=(0,1,4))
...
def estimate_full_scan_cart(radar_cart_img, model):
  sec = int(radar_cart_img.shape[0]/256)
  radar_cart_img_result = np.zeros_like(radar_cart_img)
  radar_cart_img_result_viz = np.zeros_like(radar_cart_img)
  for i in range(sec):
    for j in range(sec):
      radar_cart_img_part = radar_cart_img[i*256:(i+1)*256, j*256:(j+1)*256]
      cat_pred_, radar_cart_img_part_result = network_pred(radar_cart_img_part, model)
...
def vis_result(data_path, gt_path, gt_cart32_path, gt_cart64_path, trained_model_path, disp, save, ax1=-1, ax2=-1, ax3=-1, ax4=-1):
  ...
  for count in range(len(data_dirs)):
    ...
    gt_cart_img = gt_cart32_img
    gt_cart_img_f32 = gt_cart64_img[:256,128:128*3] ### front 32m
    ...
    radar_cart_img = radar_cart32_img
    radar_cart_img_f32 = radar_cart64_img[:256,128:128*3] ### front 32m
    #radar_cart_img = radar_cart_full_img

    sec = 5
    max_dis = 32*sec
    radar_img = radar_img_raw[:,:round(max_dis/0.0432)] # crop to 32*sec m
    radar_img = cv2.resize(radar_img, (256*sec,256))
    print('radar_img.shape', radar_img.shape)
    gt_img = np.concatenate((gt_img, np.zeros((256,512))),axis=1)
    gt_img[gt_img>=0.5]=1
    gt_img[gt_img<0.5]=0
    print('gt_img.shape', gt_img.shape)

    ## gt thres by power mask
    radar_power_mask = np.copy(radar_img)
    radar_power_mask[radar_power_mask>0.08]=1 #0.08
    radar_power_mask[radar_power_mask<=0.08]=0
    gt_img_power_mask = gt_img * radar_power_mask
    ...
    radar_power_mask = np.copy(radar_cart_img_f32)
    radar_power_mask[radar_power_mask>0.08]=1 #0.08
    radar_power_mask[radar_power_mask<=0.08]=0
    gt_cart_img_power_mask_f32 = gt_cart_img_f32 * radar_power_mask
    ##### end processing data #####

    # full test
    #radar_img_result, radar_img_result_viz = estimate_full_scan(radar_img, model, 5)

    # full test with angel normalized
    #radar_img_result, radar_img_result_viz = estimate_full_scan_w_angle_normalized(radar_img, model, 5)

    # sliding window test
#    start_dis = 0
#    radar_img_result32, radar_img32, gt_img_power_mask32 = estimate_sliding_window_scan(radar_img, gt_img_power_mask, model, start_dis)
#    start_dis = 32
#    radar_img_result64, radar_img64, gt_img_power_mask64 = estimate_sliding_window_scan(radar_img, gt_img_power_mask, model, start_dis)

#    start_dis = 32
#    radar_img_result, radar_img, gt_img_power_mask = estimate_sliding_window_scan(radar_img, gt_img_power_mask, model, start_dis)

    # sliding window test with angel normalized
#    start_dis = 32
#    radar_img_result, radar_img, gt_img_power_mask = estimate_sliding_window_scan_w_angle_normalized(radar_img, gt_img_power_mask, model, start_dis)

    # full cart test
#    radar_cart_img_result, radar_cart_img_result_viz = estimate_full_scan_cart(radar_cart_full_img, model)

    # cart test
    cat_pred_, radar_cart_img_result = network_pred(radar_cart_img, model)
#    cat_pred_, radar_cart_img_result_f32 = network_pred(radar_cart_img_f32, model)

    ### error calculation ###
    # polar
#    mask = np.ones_like(radar_img_result32)
#    calc_err(radar_img_result32, gt_img_power_mask32, mask, device, class_err_avg, dice_err_avg)
#    mask = np.ones_like(radar_img_result64)
#    calc_err(radar_img_result64, gt_img_power_mask64, mask, device, class_err_avg_1, dice_err_avg_1)
#    mask = np.ones_like(radar_img_result64)
#    mask[32:256-32,:] = 0 # mask for 32~64 front area
#    calc_err(radar_img_result64, gt_img_power_mask64, mask, device, class_err_avg_2, dice_err_avg_2)

    # cart
#    mask = np.ones_like(radar_cart_img_result)
#    calc_err(radar_cart_img_result, gt_cart_img_power_mask, mask, device, class_err_avg_3, dice_err_avg_3)

#    mask = np.ones_like(radar_cart_img_result_f32)
#    calc_err(radar_cart_img_result_f32, gt_cart_img_power_mask_f32, mask, device, class_err_avg, dice_err_avg)
#    mask = np.ones_like(radar_cart_img_result_f32)
#    mask[int(mask.shape[0]/2):,:] = 0 # mask for 32~64 front area
#    calc_err(radar_cart_img_result_f32, gt_cart_img_power_mask_f32, mask, device, class_err_avg_1, dice_err_avg_1)
#    mask = np.ones_like(radar_cart_img_result_f32)
#    mask[:int(mask.shape[0]/2),:] = 0 # mask for 0~32 front area
#    calc_err(radar_cart_img_result_f32, gt_cart_img_power_mask_f32, mask, device, class_err_avg_2, dice_err_avg_2)

#    valid_count = np.count_nonzero(gt_cart64_img) # gt_img_power_mask gt_cart32_img
#    invalid_count = 512*512 - valid_count
#    print('proper weight:', invalid_count/valid_count)
#    weight_avg.update(invalid_count/valid_count)
#    print(weight_avg)
...
#      for i in range(5):
#        j=i+1
#        radar_img_raw[:,741*j-3:741*j+3] = np.max(radar_img_raw)
#      radar_img_raw_cart = polar_to_cart_test(radar_img_raw, 0.25, 256*5, 0.0432, 2*math.pi/400).squeeze()
#      for i in range(5):
#        j=i+1
#        radar_img[:,256*j-2:256*j+2] = np.max(radar_img)
#      radar_img_cart = polar_to_cart_test(radar_img, 0.25, 256*5, 32/256, 2*math.pi/256).squeeze()
#      gt_img_power_mask_cart = polar_to_cart_test(gt_img_power_mask, 0.25, 256*5, 32/256, 2*math.pi/256).squeeze()
#      radar_img_result_cart = polar_to_cart_test(radar_img_result, 0.25, 256*5, 32/256, 2*math.pi/256).squeeze()
#      radar_img_result_cart_viz = polar_to_cart_test(radar_img_result_viz, 0.25, 256*5, 32/256, 2*math.pi/256).squeeze()

      # sliding window test
#      n = round(start_dis/0.125)
#      empty = np.zeros((256, n))
#      viz_sec = int(start_dis/32)+1
#      radar_img_cart = polar_to_cart_test(np.concatenate((empty, radar_img), axis=1), 0.25, 256*viz_sec, 32/256, 2*math.pi/256).squeeze()
#      gt_img_power_mask_cart = polar_to_cart_test(np.concatenate((empty, gt_img_power_mask), axis=1), 0.25, 256*viz_sec, 32/256, 2*math.pi/256).squeeze()
#      gt_img_power_mask_cart[gt_img_power_mask_cart>0]=1
#      radar_img_result_cart = polar_to_cart_test(np.concatenate((empty, radar_img_result), axis=1), 0.25, 256*viz_sec, 32/256, 2*math.pi/256).squeeze()
#      radar_img_result_cart[radar_img_result_cart>0]=1

      # polar
      #plot_res_polar = viz_err_fig(radar_img, gt_img_power_mask, radar_img_result)[:,:,::-1] #rgb2bgr
      #plot_res_cart = viz_err_fig(radar_img_cart, gt_img_power_mask_cart, radar_img_result_cart)[:,:,::-1] #rgb2bgr
      #plot_res_cart = viz_err_fig(radar_img_cart, radar_img_result_cart, radar_img_result_cart)[:,:,::-1] #rgb2bgr

#      sec = int(radar_cart_full_img.shape[0]/256)
#      radar_cart_full_img_viz = np.copy(radar_cart_full_img)
#      for i in range(sec):
#        radar_cart_full_img_viz[:,i*256] = 1 # = radar_cart_img[i*256:(i+1)*256, j*256:(j+1)*256]
#        radar_cart_full_img_viz[i*256,:] = 1

      # cart
      plot_res_cart = viz_err_fig(radar_cart_img, gt_cart_img, gt_cart_img_power_mask)[:,:,::-1] #rgb2bgr
      #plot_res_cart = viz_err_fig(radar_cart_full_img_viz, radar_cart_img_result, radar_cart_img_result)[:,:,::-1] #rgb2bgr

#      cv2.namedWindow('viz',cv2.WINDOW_NORMAL)
      #cv2.namedWindow('viz2',cv2.WINDOW_NORMAL)
      cv2.namedWindow('viz3',cv2.WINDOW_NORMAL)
#      cv2.imshow('viz', plot_res_polar)
      #cv2.imshow('viz2', radar_cart_img_result_viz)
      cv2.imshow('viz3', plot_res_cart)
      cv2.waitKey(0)

#      if save:
#        cv2.imwrite(os.path.join('/mnt/Disk1/viz_polar/polar_w10/full/cart_radar_full/' + str(count) + '.png'), radar_img_cart*2.*255.)
#        cv2.imwrite(os.path.join('/mnt/Disk1/viz_polar/polar_w10/full/cart_res_full/' + str(count) + '.png'), radar_img_result_cart_viz*255.)
#        cv2.imwrite(os.path.join('/mnt/Disk1/viz_polar/polar_w10/full/cart_full/' + str(count) + '.png'), plot_res_cart*255.)
#        cv2.imwrite(os.path.join('/mnt/Disk1/viz_polar/2021-05-24_22-43-05_ep10/full/polar_full/' + str(count) + '.png'), plot_res_polar*255.)

#        cv2.imwrite(os.path.join('/mnt/Disk1/viz_polar/cart_w5/full/cart_full/' + str(count) + '.png'), plot_res_cart*255.)
#        cv2.imwrite(os.path.join('/mnt/Disk1/viz_polar/cart_w5/full/cart_res_full/' + str(count) + '.png'), radar_cart_img_result_viz*255.)
...
if __name__ == "__main__":
    ...
    dice_err_avg = AverageMeter('dice_err', ':.7f')

    class_err_avg_1 = AverageMeter('class_err1', ':.7f')
    dice_err_avg_1 = AverageMeter('dice_err1', ':.7f')

    class_err_avg_2 = AverageMeter('class_err2', ':.7f')
    dice_err_avg_2 = AverageMeter('dice_err2', ':.7f')

    class_err_avg_3 = AverageMeter('class_err3', ':.7f')
    dice_err_avg_3 = AverageMeter('dice_err3', ':.7f')
...
#        cv2.namedWindow('viz',cv2.WINDOW_NORMAL)
#        cv2.namedWindow('viz2',cv2.WINDOW_NORMAL)
        dataset = '2019-01-10-12-32-52' # '2019-01-10-12-32-52' # '2019-01-10-11-46-21'
